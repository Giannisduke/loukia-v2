(function($) {
    ...
    FWP.hooks.addAction('facetwp/refresh/checkboxes_custom', function($this, facet_name) {
        var selected_values = [];
        $this.find('.facetwp-checkbox.checked').each(function() {
            selected_values.push($(this).attr('data-value'));
        });
    ...
    FWP.hooks.addFilter('facetwp/selections/checkboxes_custom', function(output, params) {
        var choices = [];
        $.each(params.selected_values, function(idx, val) {
            var choice = params.el.find('.facetwp-checkbox[data-value="' + val + '"]').clone();
            choice.find('.facetwp-counter').remove();
    ...
    $(document).on('click', '.facetwp-type-checkboxes_custom .facetwp-expand', function(e) {
        var $wrap = $(this).parent('.facetwp-checkbox').next('.facetwp-depth');
        $wrap.toggleClass('visible');
        var content = $wrap.hasClass('visible') ? FWP_JSON['collapse'] : FWP_JSON['expand'];
        $(this).html(content);
    ...
    $(document).on('click', '.facetwp-type-checkboxes_custom .facetwp-checkbox:not(.disabled)', function() {
        var $cb = $(this);
        var is_checked = ! $cb.hasClass('checked');
        var is_child = $cb.closest('.facetwp-depth').length > 0;
        var is_parent = $cb.next().hasClass('facetwp-depth');
    ...
    $(document).on('click', '.facetwp-type-checkboxes_custom .facetwp-toggle', function() {
        var $parent = $(this).closest('.facetwp-facet');
        $parent.find('.facetwp-toggle').toggleClass('facetwp-hidden');
        $parent.find('.facetwp-overflow').toggleClass('facetwp-hidden');
    });
    ...
    $(document).on('facetwp-loaded', function() {
        $('.facetwp-type-checkboxes_custom .facetwp-overflow').each(function() {
            var num = $(this).find('.facetwp-checkbox').length;
            var $el = $(this).siblings('.facetwp-toggle:first');
            $el.text($el.text().replace('{num}', num));
        });
        ...
        // add toggle feature
        $('.facetwp-type-checkboxes_custom').each(function() {
            var $facet = $(this);
            var name = $facet.attr('data-name');
    ...
    /* ======== Checkboxes Custom Icon ======== */

    FWP.hooks.addAction('facetwp/refresh/checkboxes_custom_icon', function($this, facet_name) {
        var selected_values = [];
        $this.find('.facetwp-checkbox.checked').each(function() {
            selected_values.push($(this).attr('data-value'));
        });
    ...
    FWP.hooks.addFilter('facetwp/selections/checkboxes_custom_icon', function(output, params) {
        var choices = [];
        $.each(params.selected_values, function(idx, val) {
            var choice = params.el.find('.facetwp-checkbox[data-value="' + val + '"]').clone();
            choice.find('.facetwp-counter').remove();
    ...
    $(document).on('click', '.facetwp-type-checkboxes_custom_icon .facetwp-expand', function(e) {
        var $wrap = $(this).parent('.facetwp-checkbox').next('.facetwp-depth');
        $wrap.toggleClass('visible');
        var content = $wrap.hasClass('visible') ? FWP_JSON['collapse'] : FWP_JSON['expand'];
        $(this).html(content);
    ...
    $(document).on('click', '.facetwp-type-checkboxes_custom_icon .facetwp-checkbox:not(.disabled)', function() {
        var $cb = $(this);
        var is_checked = ! $cb.hasClass('checked');
        var is_child = $cb.closest('.facetwp-depth').length > 0;
        var is_parent = $cb.next().hasClass('facetwp-depth');
    ...
    $(document).on('click', '.facetwp-type-checkboxes_custom_icon .facetwp-toggle', function() {
        var $parent = $(this).closest('.facetwp-facet');
        $parent.find('.facetwp-toggle').toggleClass('facetwp-hidden');
        $parent.find('.facetwp-overflow').toggleClass('facetwp-hidden');
    });
    ...
    $(document).on('facetwp-loaded', function() {
        $('.facetwp-type-checkboxes_custom_icon .facetwp-overflow').each(function() {
            var num = $(this).find('.facetwp-checkbox').length;
            var $el = $(this).siblings('.facetwp-toggle:first');
            $el.text($el.text().replace('{num}', num));
        });
        ...
        // add toggle feature
        $('.facetwp-type-checkboxes_custom_icon').each(function() {
            var $facet = $(this);
            var name = $facet.attr('data-name');
    ...
    FWP.hooks.addAction('facetwp/set_label/slider', function($this) {
        ...
        if ( min === max ) {
            ...
            var label = FWP.settings[facet_name]['prefix']
              ...
              var custom_label = FWP.settings[facet_name]['prefix']
                  + '<div class="input-group"><input type="text" class="form-control form-rounded" aria-label="Amount (to the nearest dollar)" value="' + nummy(min).format(format, opts) + FWP.settings[facet_name]['suffix'] +'">'
                  + '<input type="text" class="form-control form-rounded" aria-label="Amount (to the nearest dollar)" value="' + nummy(max).format(format, opts) + '">'
                  + '</div>';

              var min_label = FWP.settings[facet_name]['prefix']
              + '<input type="text" class="" aria-label="Amount (to the nearest Euro)" value="' + nummy(min).format(format, opts) + FWP.settings[facet_name]['suffix'] +'">'

              var max_label = FWP.settings[facet_name]['prefix']
              + '<input type="text" class="" aria-label="Amount (to the nearest Euro)" value="' + nummy(max).format(format, opts) + FWP.settings[facet_name]['suffix'] +'">'

        }
      //  $this.find('.facetwp-slider-label').html(custom_label);
        $this.find('.facetwp-slider-label-hidden').html(label);
        $this.find('.noUi-touch-area:eq( 0 )').html(min_label);
        $this.find('.noUi-touch-area:eq( 1 )').html(max_label);
    });

    FWP.hooks.addFilter('facetwp/selections/slider', function(output, params) {

    //  val_2 = find('.facetwp-slider-label > .input-group > .form-control:eq(1)').val();
       return params.el.find('.facetwp-slider-label-hidden').text();
    });
    ...
    FWP.hooks.addAction('facetwp/refresh/slider_indicators', function($this, facet_name) {
        FWP.facets[facet_name] = [];

        // settings have already been loaded
        if ('undefined' !== typeof FWP.frozen_facets[facet_name]) {
            if ('undefined' !== typeof $this.find('.facetwp-indicators-slider')[0].noUiSlider) {
                FWP.facets[facet_name] = $this.find('.facetwp-indicators-slider')[0].noUiSlider.get();

                // prevent changes during loading
                $this.find('.facetwp-indicators-slider')[0].setAttribute('disabled', true);
            }
        }
    });

    FWP.hooks.addAction('facetwp/loaded', function() {
        $('.facetwp-type-slider .facetwp-indicators-slider').removeAttr('disabled');
    });

    FWP.hooks.addAction('facetwp/set_label/slider_indicators', function($this) {
        var facet_name = $this.attr('data-name');
        var min = FWP.settings[facet_name]['lower'];
        var max = FWP.settings[facet_name]['upper'];
        var format = FWP.settings[facet_name]['format'];
        ...
        $this.find('.facetwp-indicators-slider-label').html(custom_label);
        $this.find('.facetwp-indicators-slider-label-hidden').html(label);
    });

    FWP.hooks.addFilter('facetwp/selections/slider_indicators', function(output, params) {

      //val_2 = find('.facetwp-indicators-slider-label > .input-group > .form-control:eq(1)').val();
       return params.el.find('.facetwp-indicators-slider-label-hidden').text();
    });

    $(document).on('facetwp-loaded', function() {
        $('.facetwp-type-slider .facetwp-indicators-slider:not(.ready)').each(function() {
            var $parent = $(this).closest('.facetwp-facet');
            var facet_name = $parent.attr('data-name');
            var opts = FWP.settings[facet_name];
            ...
            if (parseFloat(opts.range.min) >= parseFloat(opts.range.max)) {
                FWP.settings[facet_name]['lower'] = opts.range.min;
                FWP.settings[facet_name]['upper'] = opts.range.max;
                FWP.hooks.doAction('facetwp/set_label/slider_indicators', $parent);
                return;
            }

            // custom slider options
            var slider_opts = FWP.hooks.applyFilters('facetwp/set_options/slider_indicators', {
                range: opts.range,
                start: opts.start,
                step: parseFloat(opts.step),
                connect: true
        ...
        $('.facetwp-type-slider').each(function() {
            var name = $(this).attr('data-name');
            var $button = $(this).find('.facetwp-indicators-slider-reset');
            $.isEmptyObject(FWP.facets[name]) ? $button.hide() : $button.show();
        });
    });

    $(document).on('click', '.facetwp-type-slider .facetwp-indicators-slider-reset', function() {
        var facet_name = $(this).closest('.facetwp-facet').attr('data-name');
        FWP.reset(facet_name);
    });
