...
use DhlVendor\Monolog\Handler\AbstractHandler;
/**
...
class ErrorHandler
{
    ...
    public static function register(\Psr\Log\LoggerInterface $logger, $errorLevelMap = array(), $exceptionLevel = null, $fatalLevel = null)
    {
        ...
        \class_exists('DhlVendor\\Psr\\Log\\LogLevel', \true);
    ...
    {
        $this->logger->log($this->uncaughtExceptionLevel === null ? \Psr\Log\LogLevel::ERROR : $this->uncaughtExceptionLevel, \sprintf('Uncaught Exception %s: "%s" at %s line %s', \DhlVendor\Monolog\Utils::getClass($e), $e->getMessage(), $e->getFile(), $e->getLine()), array('exception' => $e));
        ...
        if ($lastError && \in_array($lastError['type'], self::$fatalErrors, \true)) {
            ...
            if ($this->logger instanceof \DhlVendor\Monolog\Logger) {
                foreach ($this->logger->getHandlers() as $handler) {
                    if ($handler instanceof \DhlVendor\Monolog\Handler\AbstractHandler) {
                        $handler->close();
                    }
                }
            }
