...
namespace DhlVendor\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use DhlVendor\WPDesk\Helper\Debug\LibraryDebug;
use DhlVendor\WPDesk\Helper\Integration\LicenseIntegration;
use DhlVendor\WPDesk\Helper\Integration\LogsIntegration;
use DhlVendor\WPDesk\Helper\Integration\SettingsIntegration;
use DhlVendor\WPDesk\Helper\Integration\TrackerIntegration;
use DhlVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use DhlVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \DhlVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    {
        $helper_info = new \DhlVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \DhlVendor\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \DhlVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \DhlVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \DhlVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \DhlVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        (new \DhlVendor\WPDesk\Helper\UpgradeSoonNotice())->show_info_about_upgrade_if_old_env();
        $this->clean_wpdesk_menu();
        $library_debug_info = new \DhlVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \DhlVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \DhlVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
