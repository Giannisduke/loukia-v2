...
class TrackerFactory
{
     ...
     * @return Tracker
     */
    public static function createCustomTracker(\DhlVendor\WPDesk\Tracker\Deactivation\PluginData $plugin_data, $scripts = null, $thickbox = null, $ajax = null)
    {
        if (empty($scripts)) {
            $scripts = new \DhlVendor\WPDesk\Tracker\Deactivation\Scripts($plugin_data);
        }
        if (empty($thickbox)) {
            $thickbox = new \DhlVendor\WPDesk\Tracker\Deactivation\Thickbox($plugin_data);
        }
        if (empty($ajax)) {
            $sender = new \DhlVendor\WPDesk_Tracker_Sender_Wordpress_To_WPDesk();
